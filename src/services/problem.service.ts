...
export class ProblemService implements IProblemService {
  ...
  async createProblem(problem: CreateProblemDto): Promise<IProblem> {
    const sanitizedPayload = {
      ...
      editorials:
        problem.editorials && (await sanitizeMarkdown(problem.editorials)),
    };
  ...
  async updateProblem(
    ...
    if (updateData.editorials) {
      sanitizedPayload.editorials = await sanitizeMarkdown(
        updateData.editorials,
      );
    }
